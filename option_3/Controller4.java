...
public class Controller4 implements Controller {
    ...
    Collection<String> seatNum = new TreeSet<String>();
    ...
    public Controller4() {
      ...
      gridPane.add(new Label("No. Adult Ticket(s):"), 0, 6);
      gridPane.add(new Label("£ 10.00"), 1, 6);
      gridPane.add(tfAdult, 2, 6);
      tfAdult.setText("0");
      gridPane.add(new Label("No. Child Ticket(s):"), 0, 8);
      gridPane.add(new Label("£   6.00"), 1, 8);
      gridPane.add(tfChild, 2, 8);
      tfChild.setText("0");
      gridPane.add(new Label("No. Senior Ticket(s):"), 0, 10);
      gridPane.add(new Label("£   6.00"), 1, 10);
      gridPane.add(tfSenior, 2, 10);
      tfSenior.setText("0");
      gridPane.add(new Label("No. Student Ticket(s):"), 0, 12);
      ...
      btBack.setOnMouseClicked(e -> {
          ...
          controller3.lblInfo.setText(title + " (" + certificate + ")\n" +
                                      time + ", Theatre " + screen);
          controller3.seatNum.addAll(seatNum);
      ...
      btLogout.setOnAction(event -> {
          ...
          btLogout.getScene().setRoot(controller1.getContent());
      });
    ...
    private void calculateTotal() {
      ...
      if ((adult + child + senior + student) == seatNum.size()) {
        tfTotal.setText(String.format("£%.2f", transaction.getTotal()));
      }
      else {
        tfTotal.setText("Err: Check No. Tickets");
      }
    }
    ...
    private void calculateChangeGiven() {
      ...
      if (transaction.getTotal() > cash) {
        tfChangeGiven.setText("Err: Insufficient Cash");
      }
      else {
